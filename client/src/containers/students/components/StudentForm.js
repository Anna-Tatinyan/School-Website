...
class StudentForm extends React.Component {
  ...
  validate(input) {
    (!input.firstName) ? this.setState({firstNameError: "Last name is required"}) : this.setState({firstNameError: ""});
    (!input.lastName) ? this.setState({lastNameError: "First name is required"}) : this.setState({lastNameError: ""});

    (!input.age) ? this.setState({ageError: "Age is required"}) : this.setState({ ageError: ""});

    (!input.gender) ?  this.setState({genderError: "Gender is required"}) : this.setState({genderError : ""});
    (!input.phone) ? this.setState({phoneError: "Phone is required"}) : this.setState({ phoneError: ""});
    (!input.email) ? this.setState({emailError: "Email is required"}) : this.setState({ emailError: ""});
...
const {firstNameError, lastNameError, ageError, genderError, phoneError, emailError, classIdError} = this.state;
    return (
      ...
      <h1 className="head"> Students </h1>
          <form className = "add-form">
                  ...
                  <div className="select">
                            ...
                            {<option value = '' default>  {(inputValues && inputValues.Class ? `${inputValues.Class.name}` : 'Choose a class')} </option>}
                  ...
                  {(this.props.id ?
                      ...
                      <button className = 'edit-button' onClick={(e) => {e.preventDefault(); this.validate(inputValues); this.props.onSubmit(inputValues);history.push(`../../students`)}}>
                        Save and close
                      </button>
