...
class CoursesForm extends React.Component {
  ...
  validate(input) {
    ...
    (!input.endDate && !input.startDate) ? this.setState({dateError: "Date is required"}) : this.setState({dateError: ""});

    (!input.time[0].endTime && !input.time[0].startTime) ? this.setState({ageError: "Time is required"}) : this.setState({ timeError: ""});

    (!input.classId) ?  this.setState({classIdError: "Class is required"}) : this.setState({classIdError : ""});
    (!input.teacherId) ? this.setState({teacherIdError: "Teacher is required"}) : this.setState({ teacherIdError: ""});
    (!input.time[0].weekDay) ? this.setState({weekDayError: "Week day is required"}) : this.setState({ weekDayError: ""});

  }
    render() {
    const {nameError, dateError, timeError, teacherIdError, classIdError, weekDayError} = this.state;
    ...
    return (
      ...
      <h1 className="head"> Courses </h1>
          <form className = "add-form">
                  ...
                  <input ref = 'startTime' defaultValue={(inputValues && inputValues.time[0].startTime) ? inputValues.time[0].startTime :  ''} type="time" id="time" name="time"  onChange={ e=> {inputValues.time[0].startTime=e.target.value}}/>
                  <input ref = 'endTime' defaultValue={(inputValues && inputValues.time[0].endTime) ? inputValues.time[0].endTime :  ''} type="time" id="time" name="time"  onChange={ e=> {inputValues.time[0].endTime=e.target.value}}/>
                  ...
                  <div className="select">
                        ...
                        <div className="select">
                              ...
                              <select ref = 'week' name="select"  onChange={(e)=> { inputValues.time[0].weekDay=e.target.value}} >
                              {<option value = '' default>  {(inputValues  && inputValues.time[0].weekDay ? `${inputValues.time[0].weekDay}` : 'Choose a weekDay')} </option>}

                                  {this.state.week.map((week, index) =>{
                                     return(
                                          <option value = {week.id} key = {index}  > {week} </option>
      ...
      <button className = 'edit-button' onClick={(e) => {console.log(inputValues); e.preventDefault(); this.validate(inputValues); this.props.onSubmit(inputValues);history.push(`../../courses`)}}>
        Save and close
      </button>
